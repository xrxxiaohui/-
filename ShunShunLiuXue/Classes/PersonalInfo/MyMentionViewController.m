...
@interface MyMentionViewController ()
@property(nonatomic,retain)NSMutableArray *dataArray;

@end

@implementation MyMentionViewController
...
//    self.view.backgroundColor = [UIColor yellowColor];
    ...
    mainTableView = [[UITableView alloc] initWithFrame:CGRectMake(0,64, kScreenWidth, kScreenHeight-64+20)];
...
-(void)fetchQuestionList{
    ...
    NSString *uid = [self userIDs];
    NSDictionary *tempDic = [NSDictionary dictionaryWithObjectsAndKeys:pageStr,@"page",uid,@"uid",tokenStr,@"token", nil];
    
    [commonModel requestUserQuestion:tempDic httpRequestSucceed:@selector(requestMyQuestionSuccess:) httpRequestFailed:@selector(requestMyQuestionFailed:)];
}
...
-(void)requestMyQuestionSuccess:(ASIHTTPRequest *)request{
    ...
    if([loginStatus isEqualToString:@"01"]){
        ...
        NSMutableArray *tempArray = [NSMutableArray arrayWithArray:[businessDataDic objectForKey:@"user_question"]];
