...
- (void)reportStatus
{
   ...
   // CLog(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", total - _skippedCount, _skippedCount, CFAbsoluteTimeGetCurrent() - _startedTime);
}
...
{
    _finishedCount++;
   // CLog(@"Prefetched %d out of %d", _finishedCount, [self.prefetchURLs count]);
...
{
    _finishedCount++;
 //   CLog(@"Prefetched %d out of %d (Failed)", _finishedCount, [self.prefetchURLs count]);
