...
- (void)initValues
{
    ...
    self.topScrollView.backgroundColor = COLOR(0xe6, 0x32, 0x14);
...
//                                              self.bounds.size.width - self.rigthSideButton.bounds.size.width, kHeightOfTopScrollView);
        ...
        [self.topScrollView setContentOffset:CGPointMake((_userSelectedChannelID - 100)*self.bounds.size.width/3, 0) animated:NO];
        //调整顶部滚动视图选中按钮位置
...
//    [self.topScrollView addSubview:menuButton];
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        CGSize textSize = [buttonTitle sizeWithFont:[UIFont systemFontOfSize:17]
        ...
        [button setFrame:CGRectMake(xOffset,0,
                                    textSize.width+10, kHeightOfTopScrollView)];
        [button setTag:i+100];
...
//        [button setBackgroundColor:COLOR(0xe6, 0x32, 0x14)];
        xOffset +=kScreenBounds.size.width/[_viewArray count];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(24, 42,  _shadowImage.size.width, _shadowImage.size.height);
            button.selected = YES;
        }
...
//                        }
//                        else{
                            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x-5, 42, _shadowImage.size.width, _shadowImage.size.height)];
//                        }
//                    }
//                        
//                }else{
