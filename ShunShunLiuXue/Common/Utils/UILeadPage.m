...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
      if (iPhone5){
        explainView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, kScreenBounds.size.width, 550)];
        explainView.image = [[UIImage imageNamed:@"explain5.png"]stretchableImageWithLeftCapWidth:0.0 topCapHeight:5.0];
      }
      else{
        explainView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, kScreenBounds.size.width, 460)];
        ...
        if (iPhone5)
          startBtn.frame = CGRectMake(0, 0, kScreenBounds.size.width, 550);
        else
          startBtn.frame = CGRectMake(0, 0, kScreenBounds.size.width, 460);
