...
@synthesize locationAttributes=_locationAttributes;
...
//创建地区对象
- (LocationAttributes *)locate
{
    if (_locationAttributes == nil) {
        _locationAttributes = [[LocationAttributes alloc] init];
    }
    
    return _locationAttributes;
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    switch (component) {
        case 0:
            ...
            [self.locatePickerView selectRow:0 inComponent:1 animated:YES];
            [self.locatePickerView reloadComponent:1];
            
            areas = [[cities objectAtIndex:0] objectForKey:@"areas"];
            [self.locatePickerView selectRow:0 inComponent:2 animated:YES];
            [self.locatePickerView reloadComponent:2];
