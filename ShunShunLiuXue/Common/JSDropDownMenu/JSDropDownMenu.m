...
//#define BackColor [UIColor colorWithRed:255.0f/255.0f green:62.0f/255.0f blue:48.0f/255.0f alpha:1.0]
#define BackColor [UIColor colorWithRed:0xe6/255.0f green:0x32/255.0f blue:0x14/255.0f alpha:1.0]
// 选中颜色加深
#define SelectColor [UIColor colorWithRed:0xe6/255.0f green:0x32/255.0f blue:0x14/255.0f alpha:1.0]
...
- (CALayer *)createBgLayerWithColor:(UIColor *)color andPosition:(CGPoint)position {
    ...
    layer.bounds = CGRectMake(0, 0, self.frame.size.width/self.numOfMenu, self.frame.size.height);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (leftOrRight == 1) {
        //右边tableview
        CGFloat marginX = 15;//右侧右边tableview的label与左边tableview的间距
        
        titleLabel.frame = CGRectMake(marginX, 0, textSize.width, cell.frame.size.height);
        
        //        cell.backgroundColor = [UIColor colorWithRed:244./255. green:244./255. blue:244./255. alpha:1];
        ...
        if ([titleLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
            ...
            accessoryImageView.frame = CGRectMake(150*self.window.frame.size.width/320,(self.frame.size.height-12)/2, 16, 12);
        ...
        if (!_hadSelected && _leftSelectedRow == indexPath.row) {
            ...
            if(!haveRightTableView){
                ...
                //                accessoryImageView.frame = CGRectMake(titleLabel.frame.origin.x+titleLabel.frame.size.width+10, (self.frame.size.height-12)/2, 16, 12);
                accessoryImageView.frame = CGRectMake(self.frame.size.width-36,(self.frame.size.height-12)/2, 16, 12);
                //左边的tableview选中字的颜色
                titleLabel.textColor = [UIColor redColor];
                [cell addSubview:accessoryImageView];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
        ...
        //是否有右边多行
        BOOL haveRightTableView = [_dataSource haveRightTableViewInColumn:_currentSelectedMenudIndex];
        if(_leftSelectedRow==0 &&leftOrRight==0){
        
            haveRightTableView = NO;
        }
